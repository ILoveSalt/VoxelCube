...
![Language](https://img.shields.io/badge/language-C%2B%2B20%20%7C%20Python%203.11-005A9E)
...
**VoxelCube Engine** — это полноценная платформа разработки воксельных игр с собственным редактором в стиле Unreal Engine, встроенным компилятором проектов, поддержкой мультиплеерных серверов и двуязычным API на **C++** и **Python**.
...
| Скриптинг | Java/Lua | Нет | **C++ / Python** |
...
- Импорт форматов:
  ...
  - Скрипты: `.cpp`, `.h`, `.py`
- Hot Reload шейдеров и Python-скриптов без перезапуска редактора
...
- Подсветка синтаксиса C++ и Python
- Автодополнение через Language Server Protocol (LSP)
- Интеграция с VS Code, CLion, PyCharm через генерацию `.vscode` / `.idea`
- Live-дебаггинг: точки останова, просмотр переменных прямо в редакторе
- Hot Reload Python-скриптов в Play Mode
...
├── Obfuscate Python: [✓] Скомпилировать .py → .pyc + шифрование
...
└── Windows_x64/
    ...
    └── Scripts/             — скомпилированные Python-скрипты
```
 
### Поддержка платформ
 
| Платформа | C++ | Python | Графика | Статус |
|---|---|---|---|---|
...
```python
# Python — тот же API
class Player(vc.Entity):
    health   = vc.Replicated(100.0)
    position = vc.Replicated(vc.Vec3())
    ...
    def play_hit_effect(self):
        self.get_component(vc.ParticleSystem).emit("hit_sparks")
```
...
VoxelCube предоставляет полноценное двуязычное API: **C++** для высокопроизводительной логики и **Python** для скриптинга, прототипирования и моддинга.
...
```python
import voxelcube as vc
 
class MyGame(vc.Application):
    def on_init(self):
        renderer = self.get_renderer()
        renderer.vsync = True
 
        self.world = vc.World.create(
            seed=42,
            generator=vc.WorldGen.PERLIN,
            merge_threshold=64,
            destruct_mode=vc.Destructibility.CHUNK_BREAK
        )
 
        self.world.voxel_registry.register("stone", {
            "texture":    "textures/stone.dds",
            "hardness":   2.5,
            "sound_group": "stone"
        })
 
        player = self.world.spawn_entity("Player")
        player.add_component(vc.CharacterController, speed=5.0)
        player.add_component(vc.Inventory, slots=36)
        player.add_component(MyPlayerScript)
 
    def on_update(self, dt: float):
        self.world.update(dt)
 
 
class MyPlayerScript(vc.Script):
    def on_update(self, dt: float):
        hit = self.owner.world.raycast(
            origin=self.owner.position,
            direction=self.owner.forward,
            max_dist=5.0
        )
        if hit and vc.Input.is_just_pressed(vc.Key.E):
            hit.entity.interact(self.owner)
...
```python
# Python
@vc.on_event(vc.VoxelDestroyedEvent)
def on_voxel_destroyed(event):
    spawn_particles(event.position, event.voxel_type)
    award_points(10)
```
...
| Python 3.11 Embed | Embed package | Скрипты |
| pybind11 | submodule | C++↔Python биндинги |
| entt | submodule | ECS |
...
│       └── scripting/          # Python embed, pybind11 биндинги
│
...
│   └── postprocess/
│
├── scripts/                    # Утилиты сборки (PowerShell / Python)
│
...
- [ ] Hot Reload шейдеров и Python-скриптов
...
Стиль кода: `.clang-format` (C++) и `black` + `ruff` (Python):
 
```bash
clang-format -i **/*.cpp **/*.h
black scripts/ && ruff check scripts/
```
